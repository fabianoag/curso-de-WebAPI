		...
		function Cadastrar() {
			...
			aluno.ra = parseInt(document.querySelector('#ra').value);

			if (aluno.id === undefined || aluno.id === 0) 
			{
				//Cadastra um novo usuário
				salvarEstudante('POST', aluno);
			}
			else
			{
				//Atualiza um usuario já existente
				salvarEstudante('PUT', aluno);
			}
		...
		function carregaEstudantes() {
			...
			xhr.open('GET',`http://localhost:53387/api/aluno/Recuperar`, true);
		...
		function salvarEstudante(metodo, corpo) {
			var xhr = new XMLHttpRequest();
			/**
			...
			*/
			...
			xhr.open(metodo,`http://localhost:53387/api/aluno`, false);
		...
		function excluirEstudante(id) {
			...
			xhr.open('DELETE',`http://localhost:53387/api/aluno/${id}`, false);
			xhr.send();
		}
